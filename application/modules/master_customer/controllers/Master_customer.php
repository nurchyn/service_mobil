...
class Master_customer extends CI_Controller {
	...
	public function index()
	{
		 ...
		 */
		$data = array(
			'title' => 'Pengelolaan Data Customer',
			'data_user' => $data_user,
			'data_role'	=> $data_role
		);
		...
		$content = [
			'css' 	=> null,
			'modal' => 'modal_master_customer',
			'js'	=> 'master_customer.js',
			'view'	=> 'view_master_customer'
		];
	...
	{
		$list = $this->m_customer->get_datatable_user();
		$data = array();
		$no =$_POST['start'];
			...
			$str_aksi = '
				<div class="btn-group">
					...
					<div class="dropdown-menu">
						<button class="dropdown-item" onclick="edit_customer(\''.$customer->id.'\')">
							<i class="la la-pencil"></i> Edit Data Customer
						</button>
						<button class="dropdown-item" onclick="delete_customer(\''.$customer->id.'\')">
	...
	public function add_data_customer()
	{
		...
		$insert = $this->m_customer->save($data_user);
		
		if ($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			$retval['status'] = false;
			$retval['pesan'] = 'Gagal menambahkan Data Customer';
		}else{
			$this->db->trans_commit();
			$retval['status'] = true;
			$retval['pesan'] = 'Sukses menambahkan Data Customer';
		}
	...
	public function update_data_customer()
	{
		...
		$where = ['id' => $id];
		$update = $this->m_customer->update($where, $data_user);

		if ($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			$data['status'] = false;
			$data['pesan'] = 'Gagal update Master Customer';
		}else{
			$this->db->trans_commit();
			$data['status'] = true;
			$data['pesan'] = 'Sukses update Master Customer';
		}
	...
	{
		$id = $this->input->post('id');
		$del = $this->m_customer->softdelete_by_id($id);
		if($del) {
			$retval['status'] = TRUE;
			$retval['pesan'] = 'Data Master Customer dihapus';
		}else{
			$retval['status'] = FALSE;
			$retval['pesan'] = 'Data Master Customer dihapus';
		}
	...
	private function rule_validasi($is_update=false, $skip_pass=false)
	{
		...
		if ($this->input->post('nama') == '') {
			...
			$data['error_string'][] = 'Wajib mengisi Nama Customer';
			$data['status'] = FALSE;
		}

		if ($this->input->post('alamat') == '') {
			$data['inputerror'][] = 'alamat';
			$data['error_string'][] = 'Wajib mengisi Alamat Customer';
			$data['status'] = FALSE;
		}
