...
class Master_mekanik extends CI_Controller {
	...
	public function index()
	{
		 ...
		 */
		$data = array(
			'title' => 'Pengelolaan Data Mekanik',
			'data_user' => $data_user,
			'data_role'	=> $data_role
		);
		...
		$content = [
			'css' 	=> null,
			'modal' => 'modal_master_mekanik',
			'js'	=> 'master_mekanik.js',
			'view'	=> 'view_master_mekanik'
		];

		$this->template_view->load_view($content, $data);
	}

	public function list_mekanik()
	{
		$list = $this->m_mekanik->get_datatable_user();
		$data = array();
		$no =$_POST['start'];
		foreach ($list as $mekanik) {
			...
			//loop value tabel db
			$row[] = $no;
			$row[] = $mekanik->nama_mekanik;
			$row[] = $mekanik->alamat;
			$row[] = $mekanik->email;
			$row[] = $mekanik->no_telp;
			$jk = ($mekanik->jenis_kelamin == 'L') ? 'Laki-laki' : 'Perempuan';
			$row[] = $jk;
			...
			$str_aksi = '
				<div class="btn-group">
					...
					<div class="dropdown-menu">
						<button class="dropdown-item" onclick="edit_mekanik(\''.$mekanik->id.'\')">
							<i class="la la-pencil"></i> Edit Data Mekanik
						</button>
						<button class="dropdown-item" onclick="delete_mekanik(\''.$mekanik->id.'\')">
		...
		$output = [
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->m_mekanik->count_all(),
			"recordsFiltered" => $this->m_mekanik->count_filtered(),
			"data" => $data
		];
	...
	public function edit_mekanik()
	{
		...
		$where = ['m_mekanik.id' => $id];

		$oldData = $this->m_global->getSelectedData('m_mekanik', $where)->row();
		if(!$oldData){
	...
	public function add_data_mekanik()
	{
		$this->load->library('Enkripsi');
		...
		if(isset($_FILES['foto']['name']) && in_array($_FILES['foto']['type'], $file_mimes)) {
			...
			if ($this->file_obj->do_upload('foto')) 
			{
				...
				$namafileseo = $this->seoUrl($namafileseo).'.'.$extDet;
			} else {
		...
		$insert = $this->m_mekanik->save($data_user);
		
		if ($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			$retval['status'] = false;
			$retval['pesan'] = 'Gagal menambahkan Mekanik';
		}else{
			$this->db->trans_commit();
			$retval['status'] = true;
			$retval['pesan'] = 'Sukses menambahkan Mekanik';
		}

		echo json_encode($retval);
	}

	public function update_data_mekanik()
	{
		...
		$id = $this->input->post('id');
		$nama_mekanik = $this->input->post('nama');
		$alamat = $this->input->post('alamat');
		$email = $this->input->post('email');
		$hp = $this->input->post('hp');
		$jenis_kelamin = $this->input->post('jenis_kelamin');
		$namafileseo = $this->seoUrl($nama_mekanik.' '.time());
		...
		if($foto != null) {
			$data_user['img_foto'] = $foto;
		}

		$where = ['id' => $id];
		$update = $this->m_mekanik->update($where, $data_user);

		if ($this->db->trans_status() === FALSE){
			$this->db->trans_rollback();
			$data['status'] = false;
			$data['pesan'] = 'Gagal update Master Mekanik';
		}else{
			$this->db->trans_commit();
			$data['status'] = true;
			$data['pesan'] = 'Sukses update Master Mekanik';
		}
	...
	{
		$id = $this->input->post('id');
		$del = $this->m_mekanik->softdelete_by_id($id);
		if($del) {
			$retval['status'] = TRUE;
			$retval['pesan'] = 'Data Master Mekanik dihapus';
		}else{
			$retval['status'] = FALSE;
			$retval['pesan'] = 'Data Master Mekanik dihapus';
		}
