...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_kendaraan_masuk').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "kendaraan/list_kendaraan_masuk",
			type : "POST" 
		},
    ...
    $('#customer').on('change', function() {
        var id = this.value
        $.ajax({
            url : base_url + 'kendaraan/getCustomer',
            type: "POST",
            dataType: "JSON",
            data : {id : id},
            success: function(data)
            {
                $('#nama_customer').val(data.old_data.nama_customer);
                $('#alamat_customer').val(data.old_data.alamat);
                $('#telp').val(data.old_data.telp);
            },
    ...
    $('#kendaraan').on('change', function() {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                $('#nama_kendaraan').val(data.old_data.nama_kendaraan);
                $('#nopol').val(data.old_data.nopol);
                $('#merek').val(data.old_data.merek);
                $('#warna').val(data.old_data.warna);
                // $('#telp').val(data.old_data.telp);
            },
            error: function (jqXHR, textStatus, errorThrown)
            {
...
function edit_kendaraan(id)
{
    ...
    $.ajax({
        url : base_url + 'kendaraan/edit_kendaraan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="nama_customer"]').val(data.old_data.nama_customer);
            $('[name="nama_kendaraan"]').val(data.old_data.nama_kendaraan);
            $('[name="nopol"]').val(data.old_data.nopol);
            $('[name="merek"]').val(data.old_data.merk);
            $('[name="hp"]').val(data.old_data.telp);
            $('[name="warna"]').val(data.old_data.warna);
            $('[name="keluhan"]').val(data.old_data.keluhan);
            $('#modal_user_form').modal('show');
	        $('#modal_title').text('Edit Kendaaran Masuk'); 
...
function tambah_mekanik(id)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : base_url + 'kendaraan/edit_mekanik',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="id_mekanik"]').val(data.old_data.id_mekanik);
            $('#modal_mekanik_form').modal('show');
	        $('#modal_title3').text('Edit Mekanik'); 
...
function list_pekerjaan(id)
{
    // alert('kesini'); exit;
    getTable(id);
    reset_modal_form();
    $('[name="id"]').val(id);
    $('#list_pekerjaan').modal('show');
    $('#modal_title2').text('List Pekerjaan WO'); 

       
}

function tambah_onderdil(id){
    window.location.href = base_url+'kendaraan/onderdil/'+id;
}
...
function save()
{
    ...
    save_method = 'add';
    if(save_method == 'add') {
        url = base_url + 'kendaraan/add_data_kendaraan';
        txtAksi = 'Tambah Data Kendaraan Masuk';
    }else{
        url = base_url + 'kendaraan/update_data_kendaraan';
        txtAksi = 'Update Data Kendaraan Masuk';
    }
...
function save_mekanik()
{
    var url;
    var txtAksi;
    // save_method = 'add';
   
    url = base_url + 'kendaraan/update_mekanik';
    txtAksi = 'Update Mekanik';
    
    
    var form = $('#form-mekanik')[0];
    var data = new FormData(form);
...
function save_pekerjaan()
{
    ...
    url = base_url + 'kendaraan/insert_pekerjaan';
    txtAksi = 'Menambah List pekerjaan';
...
function delete_merek(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_merek/delete_merek',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
                {
                    swalConfirm.fire('Berhasil Hapus Supplier!', data.pesan, 'success');
                    table.ajax.reload();
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
...
function konfirmasi_selesai(id){
    swalConfirmDelete.fire({
        title: 'Perngatan ?',
        text: "Apakah Maintance Kendaraan ini sudah selesai ?",
        type: 'warning',
        showCancelButton: true,
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'kendaraan/konfirmasi_selesai',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
...
function hapus_pekerjaan(id)
{
  ...
  swalConfirm.fire({
    ...
    text: "ingin menghapus daftar pekerjaan ini ?",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: 'Ya !',
    cancelButtonText: 'Tidak !',
    ...
    if (result.value) {
        $.ajax({
            url : base_url + 'kendaraan/hapus_pekerjaan',
            type: "POST",
            dataType: "JSON",
            data : {id : id},
            success: function(data)
...
function getTable(id){
	...
	table =  $.ajax({
        type: 'POST',
        url: base_url + 'kendaraan/list_pekerjaan',
        data: {id:id},
