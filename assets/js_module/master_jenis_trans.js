...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_jenis_trans').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_jenis_trans/list_jenis_trans",
			type : "POST" 
		},
...
function edit_jenis_trans(id)
{
    ...
    $.ajax({
        url : base_url + 'master_jenis_trans/edit_jenis_trans',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="kode"]').val(data.old_data.kode_jenis);
            $('[name="nama"]').val(data.old_data.nama_jenis);
            $('[name="keterangan"]').val(data.old_data.keterangan);
            $('#modal_pegawai_form').modal('show');
	        $('#modal_title').text('Edit Data Jenis Transaksi'); 
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_jenis_trans/add_data_jenis_trans';
        txtAksi = 'Tambah Jenis Transaksi';
    }else{
        url = base_url + 'master_jenis_trans/update_data_jenis_trans';
        txtAksi = 'Edit Jenis Transaksi';
    }
...
function delete_jenis_trans(id){
    swalConfirmDelete.fire({
        title: 'Peringatan !',
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_jenis_trans/delete_jenis_trans',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
                {
                    swalConfirm.fire('Berhasil Hapus data jenis transaksi!', data.pesan, 'success');
                    table.ajax.reload();
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
