...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_kendaraan').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_kendaraan/list_kendaraan",
			type : "POST" 
		},
...
function add_menu()
{
	...
	$('#modal_title').text('Tambah Customer Baru'); 
}
...
function edit_kendaraan(id)
{
    ...
    $.ajax({
        url : base_url + 'master_kendaraan/edit_kendaraan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="nama"]').val(data.old_data.nama_kendaraan);
            $('[name="nopol"]').val(data.old_data.nopol);
            $('[name="warna"]').val(data.old_data.warna);
            $('[name="merek"]').val(data.old_data.merek);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_kendaraan/add_data_kendaraan';
        txtAksi = 'Tambah Data Kendaraan';
    }else{
        url = base_url + 'master_kendaraan/update_data_kendaraan';
        txtAksi = 'Edit Data Kendaraan';
    }
...
function delete_kendaraan(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_kendaraan/delete_kendaraan',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
