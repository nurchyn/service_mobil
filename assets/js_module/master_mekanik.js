...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_mekanik').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_mekanik/list_mekanik",
			type : "POST" 
		},
...
function edit_mekanik(id)
{
    reset_modal_form();
    save_method = 'update';
    //Ajax Load data from ajax
    $.ajax({
        url : base_url + 'master_mekanik/edit_mekanik',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="nama"]').val(data.old_data.nama_mekanik);
            $('[name="alamat"]').val(data.old_data.alamat);
            $('[name="jenis_kelamin"]').val(data.old_data.jenis_kelamin);
            $('[name="email"]').val(data.old_data.email);
            $('[name="hp"]').val(data.old_data.no_telp);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_mekanik/add_data_mekanik';
        txtAksi = 'Tambah Data Mekanik';
    }else{
        url = base_url + 'master_mekanik/update_data_mekanik';
        txtAksi = 'Edit Data Mekanik';
    }
...
function delete_mekanik(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_mekanik/delete_mekanik',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
