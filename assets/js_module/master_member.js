...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_member').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_member/list_member",
			type : "POST" 
		},
...
function edit_member(id)
{
    ...
    $.ajax({
        url : base_url + 'master_member/edit_member',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="hp"]').val(data.old_data.hp);
            $('[name="kode_member"]').val(data.old_data.kode_member);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_member/add_data_member';
        txtAksi = 'Tambah Member';
    }else{
        url = base_url + 'master_member/update_data_member';
        txtAksi = 'Edit Member';
    }
...
function delete_member(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_member/delete_member',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
