...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_merek').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_merek/list_merek",
			type : "POST" 
		},
...
function edit_merek(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="produsen"]').val(data.old_data.chasis);
            $('#modal_user_form').modal('show');
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_merek/add_data_merek';
        txtAksi = 'Tambah Merek Mobil';
    }else{
