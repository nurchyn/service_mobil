...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_pekerjaan').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_pekerjaan/list_pekerjaan",
			type : "POST" 
		},
...
function edit_pekerjaan(id)
{
    ...
    $.ajax({
        url : base_url + 'master_pekerjaan/edit_pekerjaan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="nama"]').val(data.old_data.nama_pekerjaan);
            $('[name="type"]').val(data.old_data.type_wo);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_pekerjaan/add_data_pekerjaan';
        txtAksi = 'Tambah Data Pekerjaan';
    }else{
        url = base_url + 'master_pekerjaan/update_data_pekerjaan';
        txtAksi = 'Edit Data Pekerjaan';
    }
...
function delete_pekerjaan(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_pekerjaan/delete_pekerjaan',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
