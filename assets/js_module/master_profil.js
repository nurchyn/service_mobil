...
function save()
{
    ...
    url = base_url + 'master_profil/add_data_profil';
    var alert = "Mengubah";
    ...
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                ...
                success: function (data) {
                    if(data.status) {
                        swal.fire("Sukses!!", "Aksi mengubah data Berhasil", "success");
...
{
    $('#modal_user_form').modal('show');
    $('#modal_title').text('Edit Foto Profile'); 
}
...
$(function(){
    ...
    var maxImageHeight = 2500;
    var photoprofile = new Dropzone('#photoprofile', {
        url: base_url + 'master_profil/upload_foto',
        autoProcessQueue: false,
        dictDefaultMessage: "<i class='icon-camera-6'></i>",
        maxFiles: 1,
        maxFilesize: 2,
        init: function() {
        ...
        accept: function(file, done) {
            file.rejectDimensions = function() { 
              alert("Please make sure the image width and height are not larger than 2500px");
    ...
    photoprofile.on("sending", function(file, xhr, formData) {
        formData.append("id", "12");
        formData.append("type", "pic");
        formData.append("segment",  "tes");
    });

    $("#button-edit-foto-profile").click(function(){
      photoprofile.processQueue();
    })

    photoprofile.on("queuecomplete", function (file) {
