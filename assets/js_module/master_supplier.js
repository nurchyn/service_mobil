...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_supplier').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_supplier/list_supplier",
			type : "POST" 
		},
...
function edit_supplier(id)
{
    ...
    $.ajax({
        url : base_url + 'master_supplier/edit_supplier',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            $('[name="nama"]').val(data.old_data.nama_supplier);
            $('[name="alamat"]').val(data.old_data.alamat_supplier);
            $('[name="telp"]').val(data.old_data.telp_supplier);
            $('#modal_user_form').modal('show');
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_supplier/add_data_supplier';
        txtAksi = 'Tambah Supplier';
    }else{
        url = base_url + 'master_supplier/update_data_supplier';
        txtAksi = 'Edit Supplier';
    }
...
function delete_supplier(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_supplier/delete_supplier',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
